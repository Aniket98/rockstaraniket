...
Links:
	...
	|_ 7. [OSDBA - Oracle Group](https://docs.oracle.com/en/database/oracle/oracle-database/19/hpdbi/standard-oracle-database-groups-for-database-administrators.html)
...
What this means? This is an Oracle Server Process (Dedicated-Server-Process).
Using OS authentication, Oracle authenticated my local connection and allowed it to connect with SYSDBA privilege. Since my oracle OS user is a member of the dba OSDBA group, Oracle allows me to connect as SYSDBA without providing a database username or password.

It uses a local connection using Bequeath (BEQ) protocol.
...
As we can see above I used fake non-existing username and password, it ignored that and connected me as 'SYS' user only (see i ran the command 'show user;'.

Also, for the OS Dedicated Server process that we saw earlier:
...
an explanation for this process is given in the famous book of Tom Kyte - [Expert Oracle Database Architecture](https://documents.uow.edu.au/~jrg/317sim/ereadings/expert-oracle-database-architecture-2nded.pdf), and also in the [Offical Forum](https://asktom.oracle.com/ords/f?p=100:11:0::::P11_QUESTION_ID:1631683800346891854).

Here are the explanation screenshots directly from book:
...
What does the message 'Connected to an Idle Instance' means?
